...
def params_setup(cmdline=None):
    ...
    # path ctrl
    parser.add_argument('--model_name', type=str, default='twitter', help='model name, affects data, model, result save path')
    parser.add_argument('--scope_name', type=str, help='separate namespace, for multi-models working together')
    parser.add_argument('--work_root', type=str, default='../Datasets', help='root dir for data, model, result save path')
    ...
    if not args.scope_name:
        args.scope_name = args.model_name
    if args.rev_model:
        args.model_name += '_bidi'  # bi-direction model
    ...
    args.model_dir = '%s/checkpoints' % args.workspace
